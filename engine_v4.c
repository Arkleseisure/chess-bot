...
float values[12] = { 1, 3, 3, 5, 9, 0, -1, -3, -3, -5, -9, 0 };
...
void check_it_works() {
	confirm_it_works();
	printf(" This is the engine code version 126.\n");
	printf(" This is engine v4.\n");
}
...
void fully_evaluate(struct Game* game) {
	...
	for (int i = 0; i < 15; i++) {
		if (!(game->piece_list[i].captured)) {
			...
			game->value += piece_square_table_mg[piece.type][piece_rank][piece_file] / 100;
		}
		if (!(game->piece_list[i + 16].captured)) {
			...
			piece_rank = rank(piece.loc);
			piece_file = file(piece.loc);
			game->value -= piece_square_table_mg[piece.type - 6][7 - piece_rank][piece_file] / 100;
		}
	}
}
...
void update_value(struct Game* game, unsigned long long move[3], int captured_piece) {
	...
	if (captured_piece != 32) {
		...
		game->value -= values[piece.type];
		if (game->to_play == 1) {
			game->value += piece_square_table_mg[piece.type - 6][7 - rank(piece.loc)][file(piece.loc)] / 100;
		}
		else {
			game->value -= piece_square_table_mg[piece.type][rank(piece.loc)][file(piece.loc)] / 100;
		}
	}
	// handles promotions
	if ((move[2] & 8) != 0) {
		...
		if (game->to_play == 1) {
			game->value -= piece_square_table_mg[0][rank1][file1] / 100;
			game->value += piece_square_table_mg[prom_piece][rank2][file2] / 100;
		}
		else {
			game->value += piece_square_table_mg[6][7 - rank1][file1] / 100;
			game->value -= piece_square_table_mg[prom_piece - 6][7 - rank2][file2] / 100;
		}
		...
		int file2 = file(move[1]);
		if (game->to_play == 1) {
			game->value -= piece_square_table_mg[piece_type][rank1][file1] / 100;
			game->value += piece_square_table_mg[piece_type][rank2][file2] / 100;
		}
		else {
			game->value += piece_square_table_mg[piece_type][7 - rank1][file1] / 100;
			game->value -= piece_square_table_mg[piece_type][7 - rank2][file2] / 100;
		}
	}
}
