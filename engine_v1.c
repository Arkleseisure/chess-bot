...
void check_it_works() {
	confirm_it_works();
	printf(" This is the engine code version 25.\n");
...
Last Modified by: Arkleseisure
*/
float minimax(struct Game* game, int depth, unsigned long long* zobrist_numbers, int move_number[1], clock_t start_time, double time_allowed, int max_depth, unsigned long long nodes[1]) {
	nodes[0]++;
	...
	for (i = 0; i < num_moves; i++) {
		...
		child_value = minimax(game, depth - 1, zobrist_numbers, child_move_number, start_time, time_allowed, max_depth, nodes);
...
void get_engine_move(struct Game* game, unsigned long long* zobrist_numbers, int* current_move_number, double time_allowed, float current_value[1], int depth[1], unsigned long long nodes[1]) {
	...
	// as long as the time hasn't ellapsed, the program will keep on calculating the minimax value at greater and greater depth
	while (((double)current_time - (double)start_time) / CLOCKS_PER_SEC < time_allowed) {
		value = minimax(game, depth[0], zobrist_numbers, move_number, start_time, time_allowed, depth[0], nodes);
