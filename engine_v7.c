...
void check_it_works() {
	confirm_it_works();
	printf(" This is the engine code version 203.\n");
	printf(" This is engine v7.\n");
}
...
float minimax(struct Game* game, int depth, unsigned long long* zobrist_numbers, int move_number[1], clock_t start_time, double time_allowed, 
	...
	if (hash_table_entry) {
		for (i = 0; i < 3; i++) {
			moves[num_moves][i] = moves[entry->move_number][i];
		}
		moves[entry->move_number][0] = 0;
	}
	else {
		moves[num_moves][0] = 0;
	}
	...
	// The move suggested by the hash table is added to the end of the list, so that it can be searched first.
	// This is the reason the loop runs in reverse.
	for (i = num_moves; i >= 0; i--) {
		if (moves[i][0] != 0) {
			...
			child_value = minimax(game, depth - 1, zobrist_numbers, child_move_number, start_time, time_allowed, max_depth, alpha, beta, nodes, transposition_table, table_hits);
			...
			// if next person to play is white, they will try to maximize their score
				...
				// when a hash table entry is encountered, the move suggested in the entry is searched first
				if (i < num_moves) {
					move_number[0] = i;
				}
				else {
				...
				if (node_value > beta) {
					add_hash_table_entry(transposition_table, game->hash, depth, 1, node_value, i);
					return node_value;
				}
			...
			// if the next person to play is black, they will try to minimize their score
				...
				if (node_value < alpha) {
					add_hash_table_entry(transposition_table, game->hash, depth, 2, node_value, i);
					return node_value;
				}
...
void get_engine_move(struct Game* game, unsigned long long* zobrist_numbers, int* current_move_number, double time_allowed, float current_value[1], int depth[1], unsigned long long nodes[1]) {
	...
	while (time_taken < time_allowed) {
		value = minimax(game, depth[0], zobrist_numbers, move_number, start_time, time_allowed, depth[0], (float)-2000, (float)2000, nodes, transposition_table, table_hits);
